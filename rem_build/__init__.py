...
class AutoApplyModel:
    ...
    def resume_to_json(self, pdf_path):
        ...
        resume_json = self.llm.get_response(
            prompt=prompt, need_json_output=True)
        return resume_json
    ...
    def user_data_extraction(self, user_data_path: str = demo_data_path, is_st=False):
        ...
        #     pass
            ...
            raise Exception(
                "Invalid file format. Please provide a PDF, JSON file or url.")
        ...
        try:
            ...
            if job_site_content:
                ...
                jd_path = utils.job_doc_name(
                    job_details, self.downloads_dir, "jd")
            ...
            print(e)
            st.write(
                "Please try pasting the job description text instead of the URL.")
    ...
    def cover_letter_generator(self, job_details: dict, user_data: dict, need_pdf: bool = True, is_st=False):
        ...
        try:
            ...
            cover_letter = self.llm.get_response(
                prompt=prompt, expecting_longer_output=True)
            ...
            if need_pdf:
                utils.text_to_pdf(
                    cover_letter, cv_path.replace(".txt", ".pdf"))
                print("Cover Letter PDF generated at: ",
                      cv_path.replace(".txt", ".pdf"))
    ...
    def resume_builder(self, job_details: dict, user_data: dict, is_st=False):
        ...
        try:
            ...
            if is_st:
                st.toast("Generating Resume Details...")
            ...
            for section in ['work_experience', 'projects', 'skill_section', 'education', 'certifications', 'achievements']:
                section_log = f"Processing Resume's {
                    section.upper()} Section..."
                if is_st:
                    st.toast(section_log)

                json_parser = JsonOutputParser(
                    pydantic_object=section_mapping[section]["schema"])
                ...
                response = self.llm.get_response(
                    prompt=prompt, expecting_longer_output=True, need_json_output=True)
                        ...
                        if response[section]:
                            if section == "skill_section":
                                resume_details[section] = [
                                    i for i in response['skill_section'] if len(i['skills'])]
                            else:
            ...
            resume_path = utils.job_doc_name(
                job_details, self.downloads_dir, "resume")
            print(f'Resume Path from Init: {resume_path} \n')
    ...
    def resume_cv_pipeline(self, job_url: str, user_data_path: str = demo_data_path):
        ...
        try:
            ...
            # Build resume
            resume_path, resume_details = self.resume_builder(
                job_details, user_data)
            # resume_details = read_json("/Users/saurabh/Downloads/JobLLM_Resume_CV/Netflix/Netflix_MachineLearning_resume.json")

            # Generate cover letter
            cv_details, cv_path = self.cover_letter_generator(
                job_details, user_data)
            ...
            for metric in ['jaccard_similarity', 'overlap_coefficient', 'cosine_similarity']:
                ...
                if metric == 'vector_embedding_similarity':
                    ...
                    user_personlization = globals()[metric](
                        llm, json.dumps(resume_details), json.dumps(user_data))
                    job_alignment = globals()[metric](llm, json.dumps(
                        resume_details), json.dumps(job_details))
                    job_match = globals()[metric](
                        llm, json.dumps(user_data), json.dumps(job_details))
                else:
                    user_personlization = globals()[metric](
                ...
                print("User Personlization Score(resume,master_data): ",
                      user_personlization)
