...
def remove_urls(list_of_strings):
    ...
    filtered_list = [string for string in list_of_strings if not re.search(
        r"https?://\S+", string)]
...
def overlap_coefficient(document1: str, document2: str) -> float:
    ...
    try:
        overlap_coefficient = float(
            len(intersection)) / min(len(words_in_document1), len(words_in_document2))
...
def cosine_similarity(document1: str, document2: str) -> float:
    ...
    cosine_similarity_score = pairwise.cosine_similarity(
        vectors[0], vectors[1])
